...
class EnrichmentModule(circ_module.circ_template.CircTemplate):
    ...
    def do_intersection(self, query_bed, base_bed, circ):
        ...
        if self.virtual_inclusion_file_path != "all":
            ...
            intersect_return = self.pre_process_intersection(intersect_return, circ)
        else:
            intersect_return = base_bed.intersect(query_bed, c=True, s=True)

        return intersect_return

    def pre_process_intersection(self, intersection_input, circ):
    ...
    def random_sample_step(self, iteration, circ_rna_bed, annotation_bed, shuffled_peaks):
        ...
        circular_intersect = self.do_intersection(shuffled_peaks[iteration], circ_rna_bed, circ=True)
        linear_intersect = self.do_intersection(shuffled_peaks[iteration], annotation_bed, circ=False)
